...
import com.google.firebase.codelab.kit.model.Login_Home_help_pages.LoginActivity;
...
public class Highscore extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mExitBut.setOnClickListener((View v) -> {
            Intent start = new Intent(Highscore.this, HomePage.class);
            startActivity(start);
            Toast.makeText(this, "Come back soon!", Toast.LENGTH_SHORT).show();
        });

        mRetryBut.setOnClickListener((View v) -> {
            Intent start = new Intent(Highscore.this, memoryGameActivity.class);
            startActivity(start);
        });


        Bundle extras = getIntent().getExtras();
        String newTime = extras.getString("timing");
        if (extras.getString("timing") == null) {
            //do nothing
        } else {
            show.setText("Latest Timing: " + newTime + "s");
        }
        ...
        mDatabaseRef.orderByChild("score").limitToFirst(5).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot postSnapshot : dataSnapshot.getChildren()) {
                    ...
                    if (dataSnapshot.getChildren().equals(newTime)) {
                        Toast.makeText(Highscore.this, "Duplicate", Toast.LENGTH_SHORT).show();
                    } else {
